    ...
    public sealed class AccountRepository : IAccountRepository
    {
        ...
        public async Task<PagedResult<AccountMasterDto>> GetAccountsAsync(long? accountId, string? accountName, long? groupId, int page, int pageSize, string sortCol, string sortOrd)
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                list.Add(new AccountMasterDto
                {
                    ...
                    StakeholderTypeName = cols.Contains("StakeholderTypeName") ? row["StakeholderTypeName"]?.ToString() : null,
                    ...
                    CountryID = cols.Contains("CountryID") && row["CountryID"] != DBNull.Value ? Convert.ToInt64(row["CountryID"]) : null,
                    CountryName = cols.Contains("CountryName") ? row["CountryName"]?.ToString() : null,
        ...
        public async Task<long> SaveAccountAsync(AccountMasterDto account)
        {
            var parameters = new[]
            {
                ...
                new SqlParameter("@Address", string.IsNullOrWhiteSpace(account.Address) ? DBNull.Value : account.Address),
                new SqlParameter("@AddressCity", DBNull.Value),
                new SqlParameter("@CountryID", (object?)account.CountryID ?? DBNull.Value),
                new SqlParameter("@ContactPerson", string.IsNullOrWhiteSpace(account.ContactPerson) ? DBNull.Value : account.ContactPerson),
                new SqlParameter("@PhoneNo", string.IsNullOrWhiteSpace(account.ContactNo) ? DBNull.Value : account.ContactNo),
                new SqlParameter("@MobileNo", string.IsNullOrWhiteSpace(account.MobileNo) ? DBNull.Value : account.MobileNo),
                new SqlParameter("@EmailID", string.IsNullOrWhiteSpace(account.EmailID) ? DBNull.Value : account.EmailID),
                new SqlParameter("@StakeholderGroup", string.IsNullOrWhiteSpace(account.StakeholderGroup) ? DBNull.Value : account.StakeholderGroup),
                new SqlParameter("@StakeholderType", string.IsNullOrWhiteSpace(account.StakeholderType) ? DBNull.Value : account.StakeholderType),
                ...
                new SqlParameter("@Website", string.IsNullOrWhiteSpace(account.Website) ? DBNull.Value : account.Website),
                new SqlParameter("@Remark", string.IsNullOrWhiteSpace(account.Remark) ? DBNull.Value : account.Remark),
                new SqlParameter("@ReturnVal", SqlDbType.Int) { Direction = ParameterDirection.Output }
            };
