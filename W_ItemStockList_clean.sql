...
begin      
    ...
    declare @BatchFilter varchar(200) = ''
    if @WarehouseID <> 0
        set @BatchFilter = ' and WarehouseId = ' + cast(@WarehouseID as varchar)
 ...
 ' isnull((select sum(FinalQuantityLeft) from Inv_ItemStockByBatch where ItemId = W_ItemStock.ItemID' + @BatchFilter + '), 0) as FinalStock, ' +
 ...
 ' isnull((select sum(isnull(Amount, 0) * (FinalQuantityLeft / NULLIF(Quantity, 0))) from Inv_ItemStockByBatch where ItemId = W_ItemStock.ItemID' + @BatchFilter + '), 0) as TotalValue ' +
