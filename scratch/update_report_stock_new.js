...
const unitAutocompleteBlock = `<MudItem xs="12" md="3">
            <MudAutocomplete Clearable="true"    MaxItems="1000" T="UnitDto" Label="Unit" Value="@(_units.FirstOrDefault(x => x.UnitId == _selectedUnitId))" ValueChanged="@((UnitDto val) => { _selectedUnitId = val?.UnitId; })" SearchFunc="@SearchUnits" ToStringFunc="@(x => x?.UnitName ?? "")" Variant="Variant.Outlined"  ResetValueOnEmptyText="true" CoerceText="true" CoerceValue="true" />
        </MudItem>`;

const mudSelectReplacement = `<MudItem xs="12" md="3">
...
content = content.replace(unitAutocompleteBlock, mudSelectReplacement);
...
const oldRows = `<MudTd DataLabel="Opening Qty" Style="text-align: right;">@context.OpeningQuantity.ToString("N2")</MudTd>
...
const newRows = `<MudTd DataLabel="Opening Qty" Style="text-align: right;">@FormatQty(context.OpeningQuantity, context.UnitId)</MudTd>
...
// Note: the original file does not have double backslash on variables, let's look at the original code:
// sb.AppendLine($"{item.StockID}\t{item.ItemName}\t{item.WarehouseName}\t{item.OpeningQuantity}\t{item.PurchasedQuantity}\t{item.IssuedQuantity}\t{item.RemovedQuantity}\t{item.FinalStock}");
const oldExportCopyLiteral = 'sb.AppendLine("StockID\\tItem Name\\tWarehouse\\tOpening Qty\\tPurchased Qty\\tIssued Qty\\tRemoved Qty\\tFinal Stock");\r\n        foreach (var item in data)\r\n        {\r\n            sb.AppendLine($"{item.StockID}\\t{item.ItemName}\\t{item.WarehouseName}\\t{item.OpeningQuantity}\\t{item.PurchasedQuantity}\\t{item.IssuedQuantity}\\t{item.RemovedQuantity}\\t{item.FinalStock}");\r\n        }';

const newExportCopyLiteral = 'sb.AppendLine("StockID\\tItem Name\\tWarehouse\\tOpening Qty\\tPurchased Qty\\tIssued Qty\\tRemoved Qty\\tFinal Stock\\tStock Value");\r\n        foreach (var item in data)\r\n        {\r\n            sb.AppendLine($"{item.StockID}\\t{item.ItemName}\\t{item.WarehouseName}\\t{FormatQty(item.OpeningQuantity, item.UnitId)}\\t{FormatQty(item.PurchasedQuantity, item.UnitId)}\\t{FormatQty(item.IssuedQuantity, item.UnitId)}\\t{FormatQty(item.RemovedQuantity, item.UnitId)}\\t{FormatQty(item.FinalStock, item.UnitId)}\\t{FormatAmount(item.TotalValue)} SRD");\r\n        }';

content = content.replace(oldExportCopyLiteral, newExportCopyLiteral)
                 .replace(oldExportCopyLiteral.replace(/\r\n/g, '\n'), newExportCopyLiteral.replace(/\r\n/g, '\n'));

// 10. Update ExportToCsvAsync string building
const oldExportCsvLiteral = 'sb.AppendLine("StockID,Item Name,Warehouse,Opening Qty,Purchased Qty,Issued Qty,Removed Qty,Final Stock");\r\n        foreach (var item in data)\r\n        {\r\n            var name = (item.ItemName ?? "").Replace("\\"", "\\"\\"");\r\n            var whName = (item.WarehouseName ?? "").Replace("\\"", "\\"\\"");\r\n            sb.AppendLine($"{item.StockID},\\"{name}\\",\\"{whName}\\",{item.OpeningQuantity},{item.PurchasedQuantity},{item.IssuedQuantity},{item.RemovedQuantity},{item.FinalStock}");\r\n        }';

const newExportCsvLiteral = 'sb.AppendLine("StockID,Item Name,Warehouse,Opening Qty,Purchased Qty,Issued Qty,Removed Qty,Final Stock,Stock Value");\r\n        foreach (var item in data)\r\n        {\r\n            var name = (item.ItemName ?? "").Replace("\\"", "\\"\\"");\r\n            var whName = (item.WarehouseName ?? "").Replace("\\"", "\\"\\"");\r\n            sb.AppendLine($"{item.StockID},\\"{name}\\",\\"{whName}\\",{FormatQty(item.OpeningQuantity, item.UnitId)},{FormatQty(item.PurchasedQuantity, item.UnitId)},{FormatQty(item.IssuedQuantity, item.UnitId)},{FormatQty(item.RemovedQuantity, item.UnitId)},{FormatQty(item.FinalStock, item.UnitId)},{FormatAmount(item.TotalValue)} SRD");\r\n        }';

content = content.replace(oldExportCsvLiteral, newExportCsvLiteral)
                 .replace(oldExportCsvLiteral.replace(/\r\n/g, '\n'), newExportCsvLiteral.replace(/\r\n/g, '\n'));
