-- ============================================================================
-- PRODUCTION DATABASE MASTER SYNC PATCH SCRIPT
-- Purpose:
...
-- 3. Fix ItemBatchNumberList, Rpt_ItemStockTraceability & Rpt_BatchTraceability so all completed batch rows are displayed properly.
-- 4. Fix Inv_ItemStockByBatchList parameter list (@SortOrd, @SortColumn) so C# API calls succeed without parameter mismatch errors.
-- 5. Enable interactive links (RefUsedFor / RefUsedForId) for Produced, Purchased, and Issued records in Stock History & Batch Traceability.
-- Assurance:
...
CREATE OR ALTER TRIGGER trg_SyncBatchQuantity ON Inv_ItemStockUsed AFTER INSERT, UPDATE, DELETE AS 
BEGIN 
    SET NOCOUNT ON; 
    UPDATE b 
...
CREATE OR ALTER PROCEDURE [dbo].[ItemBatchNumberList]
    @ItemID int=0,
...
CREATE OR ALTER PROCEDURE [dbo].[Inv_ItemStockByBatchList]        
...
CREATE OR ALTER PROCEDURE [dbo].[Rpt_ItemStockTraceability]
@ItemId bigint
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT 
        ...
        CASE 
            WHEN b.StockById = 4 THEN 3 
            WHEN b.StockById = 2 THEN 2 
            WHEN b.StockById = 1 THEN -1 
            ELSE 0 
        END AS RefUsedFor,
        CASE 
            WHEN b.StockById = 4 OR b.StockById = 2 THEN CAST(b.IdFrom AS bigint) 
            WHEN b.StockById = 1 THEN ISNULL((SELECT TOP 1 pc.PurchaseID FROM W_PurchaseChild pc WHERE pc.PurchaseItemID = b.IdFrom), CAST(0 AS bigint))
...
-- 5. UPDATE SP: Rpt_BatchTraceability
CREATE OR ALTER PROCEDURE [dbo].[Rpt_BatchTraceability]
@BatchNo NVARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;
...
-- 6. UPDATE SP: W_ItemStockList
CREATE OR ALTER PROCEDURE [dbo].[W_ItemStockList]        
    @StockID bigint=0,        
...
BEGIN        
    ...
    IF @CreatedBy <> ''        
        SET @Query = @Query + ' and W_ItemStock.CreatedBy=''' + cast(@CreatedBy as varchar) + ''''        
    ...
    IF @FromDate IS NOT NULL        
        SET @Query = @Query + ' and W_ItemStock.OpeningStockDate >= ''' + convert(varchar(10), @FromDate, 120) + ''''        
            
    IF @ToDate IS NOT NULL        
        SET @Query = @Query + ' and W_ItemStock.OpeningStockDate <= ''' + convert(varchar(10), @ToDate, 120) + ''''        
    ...
    declare @MaxPage as int          
    set @MaxPage = (@TotalRecord / @RecordPerPage)          
