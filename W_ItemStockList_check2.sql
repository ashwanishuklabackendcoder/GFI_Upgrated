...
begin      
 ...
 ' W_ItemStock.StockID, ROUND(W_ItemStock.OpeningQuantity, 2) as OpeningQuantity, ROUND(W_ItemStock.PurchasedQuantity, 2) as PurchasedQuantity, ROUND(W_ItemStock.ProducedQuantity, 2) as ProducedQuantity, W_ItemStock.ItemID, W_ItemStock.WarehouseID, W_Item
Stock.UnitId, ROUND(W_ItemStock.IssuedQuantity, 2) as IssuedQuantity, W_ItemStock.CreatedBy, W_ItemStock.OpeningStockDate, ROUND(W_ItemStock.RemovedQuantity, 2) as RemovedQuantity, ' +
...
ete = 1)) OR (b.StockById = 4 AND EXISTS (SELECT 1 FROM dbo.W_Production p WHERE p.ProductionId = b.IdFrom AND p.IsComplete = 1)))), 0) as FinalStock, ' +
 ...
 ' * (isnull(ROUND(W_ItemStock.OpeningQuantity, 2), 0) + isnull(ROUND(W_ItemStock.PurchasedQuantity, 2), 0) + isnull(ROUND(W_ItemStock.ProducedQuantity, 2), 0) - isnull(ROUND(W_ItemStock.IssuedQuantity, 2), 0) - isnull(ROUND(W_ItemStock.RemovedQuantity, 2
), 0)) end) as TotalValue ' +
